...
declare const moment: (date: string, format: string) => { format: (format: string) => string };
declare const EXIF: {
...
const moduleDefinition: Partial<ModuleInstance> = {
	...
	defaults: {
		...
		radialGradient: ['rgba(0,0,0,0) 0%', 'rgba(0,0,0,0) 75%', 'rgba(0,0,0,0.25) 100%'],
	...
	createGradientDiv(this: ModuleInstance, direction: string, gradient: string[], wrapper: HTMLElement): void {
		this.uiBuilder?.createGradientDiv(direction, gradient, wrapper);
	},

	createRadialGradientDiv(this: ModuleInstance, type: string, gradient: string[], wrapper: HTMLElement): void {
		this.uiBuilder?.createRadialGradientDiv(type, gradient, wrapper);
	},

	createImageInfoDiv(this: ModuleInstance, wrapper: HTMLElement): HTMLDivElement | null {
		return this.uiBuilder?.createImageInfoDiv(wrapper) || null;
	},

	createProgressbarDiv(this: ModuleInstance, wrapper: HTMLElement, slideshowSpeed: number): void {
			...
			setTimeout(() => {
				EXIF.getData(image, () => {
					...
					if (this.config.showImageInfo && this.imageInfoDiv) {
						...
						if (dateTime !== null) {
							try {
								const dateMoment = moment(String(dateTime), 'YYYY:MM:DD HH:mm:ss');
	...
	updateImage(this: ModuleInstance, backToPreviousImage = false, imageToDisplay: string | null = null): void {
		if (imageToDisplay) {
			this.displayImage({
	...
	updateImageInfo(this: ModuleInstance, imageinfo: ImageInfo, imageDate: string): void {
