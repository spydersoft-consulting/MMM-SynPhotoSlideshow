...
class ImageProcessor {
	...
	constructor(config: Partial<ModuleConfig>, imageCache: ImageCache | null = null) {
		this.config = config;
	 ...
	 */
	private async resizeImage(inputPath: string, callback: (data: string | null) => void): Promise<void> {
	 ...
	 */
	private async readFileRaw(filepath: string, callback: (data: string | null) => void): Promise<void> {
	...
	private async downloadSynologyImage(
		...
		try {
			...
			Log.error(`Error downloading Synology image: ${(error as Error).message}`);
			callback(null);
		}
	}
