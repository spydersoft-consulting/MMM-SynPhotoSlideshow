...
class SynologyPhotosClient {
	 ...
	 */
	private async findTagsInSpace(space: { id: number; name: string; api: string }): Promise<boolean> {
	...
	private async findTagsInMultipleSpaces(): Promise<boolean> {
		...
		for (const space of spaces) {
			try {
				...
				Log.warn(`Error fetching tags from ${space.name} space: ${(error as Error).message}`);
			}
		}
	...
	private async fetchSharedAlbumPhotos(): Promise<PhotoItem[]> {
		try {
			...
			Log.error(`Error fetching shared album photos: ${(error as Error).message}`);
			return [];
		}
	}
	 ...
	 */
	private async fetchPhotosByTagInSpace(tagId: number, spaceId: number | null): Promise<PhotoItem[]> {
		try {
			const params: Record<string, unknown> = {
	...
	private removeDuplicatePhotos(photos: PhotoItem[]): PhotoItem[] {
		...
		return photos.filter((photo) => {
			const dedupeKey = (photo as PhotoItem & { synologyId?: number; id?: number }).synologyId || (photo as PhotoItem & { id?: number }).id;
	 ...
	 */
	private processPhotoList(photos: SynologyPhoto[], spaceId: number | null = null): PhotoItem[] {
	 ...
	 */
	private getPhotoUrl(photoId: number, cacheKey: string | undefined, spaceId: number | null = null): string {
		let url: string;
