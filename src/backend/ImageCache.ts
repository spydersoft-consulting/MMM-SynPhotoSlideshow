...
type ImageDownloadCallback = (image: PhotoItem, callback: (data: string | null) => void) => void;
...
class ImageCache {
	...
	async initialize(): Promise<boolean> {
		try {
			...
			Log.error(`Failed to initialize image cache: ${(error as Error).message}`);
			return false;
		}
	}
	...
	async evictOldFiles(): Promise<void> {
		if (this.currentCacheSize <= this.maxCacheSize || !this.cacheDir || !this.cache) {
			return;
		}
		...
		try {
			...
			fileStats.push(...(allStats.filter((stat) => stat !== null) as FileStats[]));
			...
			for (const file of fileStats) {
				...
				try {
					...
					Log.debug(`Could not evict ${file.name}: ${(error as Error).message}`);
				}
			}
		} catch (error) {
	 ...
	 */
	async preloadImages(images: PhotoItem[], downloadCallback: ImageDownloadCallback): Promise<void> {
		...
		void this.processPreloadQueue(downloadCallback);
	}

	/**
	 * Process preload queue in background
	 */
	private async processPreloadQueue(downloadCallback: ImageDownloadCallback): Promise<void> {
